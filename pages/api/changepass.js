...
export default async function handler(req, res) {
  ...
  if (!user) {
    console.log(`timestamp=${now}|action=passchanged|state=FAILED|username=${user.username}|IP=${ip}|reason=NOEXIST`)
  ...
  if (!match) {
    console.log(`timestamp=${now}|action=passchanged|state=FAILED|username=${user.username}|IP=${ip}|reason=BADPASS`)
    return res.writeHead(302, {
  ...
  console.log(`timestamp=${now}|action=passchanged|state=SUCCESS|username=${user.username}|IP=${ip}`)
  // Redirect on success
  return res.writeHead(302, {
